...
Future<void> main() async {
  ...
  print('  - Total lines of code: ${summary['projectSummary']['totalLinesOfCode']}');
  print('  - Widget classes found: ${summary['widgetAnalysis']['totalWidgetClasses']}');
  print('  - Controller/Service classes: ${summary['controllerAnalysis']['totalControllerServices']}');
  print('  - Async/await usage: ${summary['asyncAnalysis']['totalAsyncAwaitUsage']}');
  print('  - Stream subscriptions: ${summary['streamAnalysis']['totalStreamSubscriptions']}');
  print('  - Max nesting depth: ${summary['projectSummary']['maxNestingDepth']}');
...
Future<CodeMetrics> analyzeFile(String filePath, Map<String, dynamic> lintReport) async {
  ...
  metrics.linesOfCode = lines.where((line) => 
  ...
  for (int i = 0; i < lines.length; i++) {
    ...
    if (trimmedLine.contains('.listen(') || 
      ...
      metrics.streamSubscriptions.add('Line ${i + 1}: ${trimmedLine.substring(0, 50)}...');
    }
  ...
  return {
    ...
    'streamAnalysis': {
      ...
      'streamFiles': allMetrics.where((m) => m.streamSubscriptions.isNotEmpty).map((m) => {
        ...
        'streams': m.streamSubscriptions.take(3).toList(), // Show first 3 examples
      }).toList(),
    },
    'asyncAnalysis': {
      ...
      'asyncFiles': allMetrics.where((m) => m.asyncAwaitUsage.isNotEmpty).map((m) => {
        ...
        'asyncUsages': m.asyncAwaitUsage.take(3).toList(), // Show first 3 examples
      }).toList(),
    },
    'codeQualityMetrics': {
      'filesWithHighComplexity': allMetrics.where((m) => m.maxNestingDepth > 4).map((m) => {
        'file': m.filePath.split('/').last,
        'nestingDepth': m.maxNestingDepth,
        'linesOfCode': m.linesOfCode,
      }).toList(),
      'largestFiles': (allMetrics..sort((a, b) => b.linesOfCode.compareTo(a.linesOfCode)))
        .take(10).map((m) => {
        'file': m.filePath.split('/').last,
        'linesOfCode': m.linesOfCode,
      }).toList(),
