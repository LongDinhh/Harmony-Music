...
class DartCodeAnalyzer extends RecursiveAstVisitor<void> {
  ...
  void visitClassDeclaration(ClassDeclaration node) {
    ...
    if (extendsClause != null) {
      ...
      } else if (superClassName == 'GetxController' ||
          superClassName == 'GetxService') {
  ...
  void visitStreamSubscription(Expression node) {
    if (node.toString().contains('.listen(')) {
      metrics.streamSubscriptions
          .add('Stream subscription at line ${node.offset}');
    }
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    if (methodName == 'listen' || methodName == 'stream') {
      metrics.streamSubscriptions
          .add('$methodName call at line ${node.offset}');
    }
...
Future<CodeMetrics> analyzeFile(AnalysisContextCollection collection,
