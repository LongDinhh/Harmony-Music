...
class ThemeController extends GetxController {
  ...
  ThemeData _createThemeData(MaterialColor? primarySwatch, ThemeType themeType,
    ...
    switch (themeType) {
      case ThemeType.dynamic:
        return _createDynamicTheme(primarySwatch!, titleColorSwatch, textColor);
      case ThemeType.dark:
        return _createDarkTheme();
      case ThemeType.light:
        return _createLightTheme();
      case ThemeType.system:
        // This case should not happen as it's handled in changeThemeModeType
        throw ArgumentError(
            'System theme type should be resolved before calling _createThemeData');
    }
  }

  /// Creates dynamic theme based on primary color swatch
  ThemeData _createDynamicTheme(MaterialColor primarySwatch,
      MaterialColor? titleColorSwatch, Color? textColor) {
    _setSystemUIOverlayForDark();
  ...
  /// Sets system UI overlay style for dark themes (dynamic and dark)
  void _setSystemUIOverlayForDark() {
  ...
  /// Sets system UI overlay style for light theme
  void _setSystemUIOverlayForLight() {
    SystemChrome.setSystemUIOverlayStyle(
      SystemUiOverlayStyle(
...
extension ComplementaryColor on Color {
  ...
  Color getComplementaryColor(Color color) {
    int r = 255 - ((color.r * 255.0).round() & 0xff);
    int g = 255 - ((color.g * 255.0).round() & 0xff);
    int b = 255 - ((color.b * 255.0).round() & 0xff);
    return Color.fromARGB(((color.a * 255.0).round() & 0xff), r, g, b);
  }
}
