...
class WidgetDepthAnalyzer {
  ...
  void analyzeDirectory(String path) {
    ...
    final dartFiles = directory
        ...
        .where((entity) => entity.path.endsWith('.dart') && entity.path.contains('ui/'))
        .cast<File>();
  ...
  void generateMarkdownReport(List<WidgetDepthInfo> flaggedFiles) {
    ...
    buffer.writeln('- **Files with depth > $maxDepthThreshold**: ${flaggedFiles.length}');
    ...
    for (int i = 0; i < flaggedFiles.length; i++) {
      ...
      if (info.maxDepth > 20) {
        buffer.writeln('- ⚠️ **Critical**: Extract nested widgets into separate components');
        buffer.writeln('- 📦 Use `RepaintBoundary` for complex subtrees');
        buffer.writeln('- 🔄 Consider `LayoutBuilder` for responsive layouts');
        buffer.writeln('- 📋 Replace deep nesting with `SliverList` for scrollable content');
        ...
        buffer.writeln('- 📦 Extract some nested widgets into separate components');
    ...
    buffer.writeln('                      child: Container( // 10+ levels deep...');
    buffer.writeln('                        child: Column(');
    ...
    buffer.writeln('          itemBuilder: (context, index) => _ItemWidget(items[index]),');
    buffer.writeln('        );');
    buffer.writeln('      },');
    buffer.writeln('    );');
    buffer.writeln('  }');
    ...
    buffer.writeln('');
    buffer.writeln('1. **Extract Components**: Break down complex widgets into smaller, focused components');
    buffer.writeln('2. **Use RepaintBoundary**: Wrap expensive widgets that don\'t need frequent rebuilds');
    buffer.writeln('3. **LayoutBuilder**: Use for responsive design instead of deep conditional nesting');
    buffer.writeln('4. **SliverList**: Replace deeply nested scrollable content with Slivers');
    buffer.writeln('5. **Const Constructors**: Use const constructors where possible to reduce rebuilds');
    buffer.writeln('6. **Builder Pattern**: Use Builder widgets to limit rebuild scope');
