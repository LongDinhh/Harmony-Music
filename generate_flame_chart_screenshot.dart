...
Future<void> main() async {
  ...
  flameCaptureText
      .writeln('🔥 FLUTTER PERFORMANCE FLAME CHART - Harmony Music App');
  flameCaptureText.writeln('=' * 80);
  flameCaptureText.writeln();
  flameCaptureText
      .writeln('Profile Duration: 30s | Total Frames: ${frames.length}');
  ...
  for (int i = 0; i < frames.length && i < 50; i++) {
    ...
    flameCaptureText.writeln(
        'F${i.toString().padLeft(3)}: |$bar| ${duration.toStringAsFixed(1)}ms$status');

    // Show top widgets in this frame
    if (widgets.isNotEmpty && i < 10) {
      // Show detail for first 10 frames
      ...
      for (int w = 0; w < topWidgets.length && w < 3; w++) {
        ...
        flameCaptureText.writeln(
            '     └─ ${widget['name']}: ${widgetDuration.toStringAsFixed(1)}ms');
      }
    }
  }
  ...
  final sortedWidgets = widgetStats.entries.toList()
    ..sort((a, b) => (b.value['totalTime'] as double)
        .compareTo(a.value['totalTime'] as double));

  flameCaptureText.writeln('TOP 10 MOST EXPENSIVE WIDGETS:');
  flameCaptureText.writeln();
  flameCaptureText.writeln(
      '${'Widget Name'.padRight(25)}${'Builds'.padRight(8)}${'Total Time'.padRight(12)}${'Avg Time'.padRight(10)}Max Time');
  ...
  final avgFrameTime =
      frames.fold(0, (sum, f) => sum + (f['duration'] as int)) /
          frames.length /
          1000;

  flameCaptureText.writeln(
      '• Janky Frames: $jankyFrames/${frames.length} (${(jankyFrames / frames.length * 100).toStringAsFixed(1)}%)');
  flameCaptureText
      .writeln('• Average Frame Time: ${avgFrameTime.toStringAsFixed(1)}ms');
  ...
  flameCaptureText
      .writeln('   2. Player control components - High frequency updates');
  flameCaptureText.writeln(
      '   3. Settings screen Obx widgets - Expensive dropdown rebuilds');
  flameCaptureText
      .writeln('   4. Large widget wrapping - 100+ line widgets in Obx');
  ...
  await File('flame_chart_screenshot.txt')
      .writeAsString(flameCaptureText.toString());
  ...
  print(
      '   - Janky frames: $jankyFrames (${(jankyFrames / frames.length * 100).toStringAsFixed(1)}%)');
